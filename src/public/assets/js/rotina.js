...
async function carregarDados() {
    ...
    try {
        ...
        const refeicoes  = respostas[0].ok ? await respostas[0].json() : [];
        const dieta      = dietaId  && respostas[1]?.ok ? await respostas[1].json() : null;
        const rotina     = rotinaId && respostas[dietaId ? 2 : 1]?.ok
        ...
        document.getElementById('grade-rotina').innerHTML = `
            <div class="aviso-vazio">
                ...
                <p>Verifique se o servidor está rodando em <strong>localhost:3000</strong>.</p>
            </div>
        `;
    }
}
